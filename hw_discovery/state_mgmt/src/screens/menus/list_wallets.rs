...
pub fn initialize_menu(mut state: &mut MMState) -> bool {
    ...
    menu_choices
        .push(Box::new("+ Add Wallet".to_string()))
        .unwrap();
    menu_choices
        .push(Box::new("Cool Wallet".to_string()))
        .unwrap();

    state.current_screen = ScreenTypes::MenuScreenType;
    state.menu_prompt = Box::new("Your Wallets:".to_string());
    state.menu_choices = menu_choices;
    state.menu_hover_index = 0;
    state.menu_type = MenuTypes::ListSeedsMenuType;
